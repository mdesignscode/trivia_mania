...
<div class="space-y-2 md:space-y-4 px-6 py-4 mx-auto md:w-[55%] md:p-8">
        <div class="flex gap-4 mb-4 md:mb-6 mx-auto w-fit items-center">
                <img class="border size-12 rounded-full aspect-square" src={$globalStore.user?.avatar} alt="User icon" />

                <div>
                        <p class="text-xl">{$globalStore.user?.username}</p>
                        <p class="italic dark:text-gray-200 text-gray-500 text-xs">@{$globalStore.user?.email}</p>
                </div>
        </div>
        <h1 class="text-center text-lg md:text-xl font-bold">Settings</h1>
	<div class="rounded-sm border md:rounded-md p-2 md:px-6 md:py-4 space-y-2">
                <!-- change avatar -->
                <div class="relative">
                        <SettingForm
                        ...
                        >
                                <input
                                        required
                                        class="mr-1 p-1 rounded-sm bg-white"
                                        type="file"
                                        accept="image/*"
                                        name="avatar"
                                />
                        ...
                        {#if $globalStore.user?.avatar !== '/images/icons8-user-64.png' && $changeAvatar.expanded}
                                <form
                                        class="px-2 absolute right-4 bottom-4"
                                        method="POST"
                                ...
                                >
                                        <input
                                                type="hidden"
                                                bind:value={$globalStore.user.username}
                                                name="username"
                                        />
                                        <Button
                                                nativeProps={{ disabled: updating }}>
                                                Set default
                                        </Button>
                                </form>
                        {/if}
...
<style>
        input {
                @apply dark:text-dark dark:bg-gray-100;
        }
</style>
