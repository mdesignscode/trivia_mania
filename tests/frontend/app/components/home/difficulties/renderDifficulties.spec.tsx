...
// mock custom hook `useInitialStats`
...
describe("RenderDifficulties component", () => {
  it("Should render a list of difficulty buttons", async () => {
    ...
    // render component
    renderHomeContext(<RenderDifficulties handleDifficulty={mockHandler} />);
    ...
    for (const key of Object.keys(mockDifficultyStats)) {
      ...
      expect(mockHandler).toBeCalledWith(value);
    }
  });
  ...
  it("snapshot matches", () => {
    ...
    const { baseElement } = renderHomeContext(
      <RenderDifficulties handleDifficulty={mockHandler} />
    );
